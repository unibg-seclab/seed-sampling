...
int parse(struct config *params, int argc, char **argv) {
        ...
        if (argc < 8) {
                printf("Missing arguments: fname[char *], page_size[size_t], seed_pages[size_t], "
                       "entropy_pages[size_t], reps[size_t]\n, key[hex "
                       "string], iv[hex string]");
                return 1;
        }
        params->fname = argv[1];
        ...
        if (parse_size_t(&params->entropy_pages, argv[4]) ||
            params->entropy_pages < params->seed_pages) {
